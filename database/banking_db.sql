...
BEGIN
    ...
    IF NEW.type = 1 THEN
        SET bal = bal + NEW.amount; -- Cash in
    ELSEIF NEW.type = 2 THEN
        SET bal = bal - NEW.amount; -- Withdraw
    ELSE
        -- For transfers
        IF NEW.account_id = (SELECT account_id FROM transactions WHERE id = NEW.id) THEN
            SET bal = bal - NEW.amount; -- Transferred from this account
        ELSE
            SET bal = bal + NEW.amount; -- Transferred to this account
        END IF;
    END IF;
    
    -- Update account balance
    UPDATE accounts SET balance = bal WHERE id = NEW.account_id;
END//
DELIMITER ;

-- View to display account details along with account owner's full name
CREATE VIEW account_details AS
SELECT 
    a.id,
    a.account_number,
    CONCAT(u.firstname, ' ', u.lastname) AS account_owner,
    a.balance,
    a.date_created,
    a.date_updated
FROM
